...
return gulp.src('../content/css/*.css')
    .pipe(concatCss("style.css"))
    .pipe(uncss({
            html: ['../content/private/html/*.html', 'http://www.colombiashopping.esy.es/index.php']
        }))
    .pipe(prefix('last 2 versions', '> 5%', 'ie 9'))
    .pipe(minifyCSS({keepBreaks:false}))
    .pipe(rename('style.min.css'))
...
gulp.task('default', ['connect-sync', 'scripts', 'styles', 'images'], function() {
    ...
    gulp.watch('../content/private/images/**/*.*', ['images']);
...
gulp.task('init', function(){
  gulp.src(['../public_html/**/*', '../public_html/.htaccess'])
    .pipe(replace('praxxis-consultores.esy.es', 'praxxis-consultores.com'))
    .pipe(gulp.dest('../public_html/'))
    .pipe(notify("Ha finalizado la task Init!"));
});
...
gulp.task('reload', function() {
	connect.server({}, function() {
		browserSync.init({
			proxy: 'http://localhost/praxxis-consultores.com/',
			port: 3000
		});
		gulp.watch(['../public_html/**/*', '../content/**/*'], reload);
	});
...
gulp.task('styles', function () {
	return gulp.src(['../content/css/**/*.css', '!../content/css/**/*.min.css'])
	    ...
	    .pipe(uncss({
		        html: ['../content/private/html/*.html']
		    }))
...
gulp.task('default', ['connect-sync', 'scripts', 'styles', 'images'], function() {
	// watch for changes
	gulp.watch('../content/css/*.css', ['styles'], reload);
    gulp.watch('../content/js/**/*.js', ['scripts'], reload);
    gulp.watch('../content/images/**/*.*', ['images'], reload);
});
